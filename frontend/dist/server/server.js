...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("./assets/_tanstack-start-manifest_v-DwUfbhEo.js");
...
async function transformManifestAssets(source, transformFn, _opts) {
  ...
  for (const route of Object.values(manifest2.routes)) {
    ...
    if (route.assets) {
      for (const asset of route.assets) if (asset.tag === "link" && ((_a = asset.attrs) == null ? void 0 : _a.href)) {
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import("./assets/router-7wdfkOG4.js"),
...
function createStartHandler(cbOrOptions) {
  ...
  const startRequestResolver = async (request, requestOpts) => {
    ...
    try {
      ...
      if (router && !cbWillCleanup) (_b = router.serverSsr) == null ? void 0 : _b.cleanup();
      router = null;
    }
  };
